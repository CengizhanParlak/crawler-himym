...
const getWriteStream = syncErrorTracer(function getWriteStream(
...
const writeLine = syncErrorTracer(function writeLine(
  doc: PDFKit.PDFDocument,
  character: string,
  dialogue: string
) {
...
const pdfSaver = syncErrorTracer(function pdfSaver(
  titleRaw: string,
  script: Script
): Promise<string | FunctionError> {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        ...
        .text(titleRaw, { align: "center" });
      doc.moveDown();

      doc.fontSize(P.fontSize);
      // In your main pdfSaver function, update the scene cue handling:
      script.forEach((item) => {
        if (item.type === "dialogue") {
          writeLine(doc, item.character, item.dialogue);
        } else if (item.type === "sceneCue") {
          writeSceneCue(doc, item.content, item.isBold);
        }
      });
...
function writeSceneCue(
  doc: PDFKit.PDFDocument,
  content: string,
  isBold: boolean
) {
  const font = isBold ? "Helvetica-Bold" : "Helvetica-Oblique";
  let wrappedContent = "";
  if (content.includes("[" || "]")) {
  ...
  doc.font(font).text(wrappedContent, { align: "left" });
  doc.moveDown(0.5);
}
...
export async function saver(scriptList: { title: string; script: Script }[]) {
  ...
  const pdfPromiseList = scriptList.map(({ title, script }) =>
    pdfSaver(title, script)
  );
  ...
  const [pdfList, errorList] = settledPdfList.reduce(
    ([p, e], promiseResult, i) => {
      ...
      if (promiseResult.status === "rejected")
        return [[...p], [...e, { title, error: promiseResult.reason }]];
      else {
        const { value } = promiseResult;
        if (value instanceof FunctionError)
          return [[...p], [...e, { title, error: value }]];
  ...
  if (errorList.length > 0) {
    ...
    errorList.forEach((e) => {
      console.log(e.title);
      e.error instanceof FunctionError
        ? console.log(e.error.trace)
        : console.log(e.error);
    });
  }
}
